  ...
  procedure TLog.OpenChatLog(bot_id:string);
    begin
      try
        ...
        _writechatlog:=true;
        Log('Log','Os logs do chat serão armazenados no arquivo: ' + bot_id + '.chatlog');
      except
        _writechatlog:=false;
        Log('Log','Não é possivel gravar o log do chat, o log sera desativado');
      end;
    end;
  ...
  Procedure TLog.Log(s:string);
    begin
      if assigned(frmJohnHammond) then
       // Comentado para tirar ao informações dos arquivos carregados
