...
implementation
    ...
    begin
      ...
      if result then begin
        ...
        while not ((Parser.CurPartType=ptEndTag) and (Parser.CurName='category')) do
          parser.Scan;
      if (PatternMatcher._count mod 5000) =0 then log.Log('CarregandorAIML',Inttostr(PatternMatcher._count)+' categorias...');
    end;
  ...
  procedure TAIMLLoader.load(filename:string);
    ...
    begin
      ...
      if findfirst(filename,0,search) =0 then
        repeat
          ...
          if not FileExists(name) then begin
            log.log('CarregandorAIML','Warning: '+Name+' not found');
            continue;
          end;
          log.log('CarregandorAIML','Carregando '+Name);
      ...
      FindClose(search);
      parser.free;
      Log.log('CarregandorAIML','Carregado ' + InttoStr(PatternMatcher._count) + ' categorias');
    end;
end.
